...
extension UIImage {
  ...
  @objc func scaledImage(with size: CGSize) -> UIImage? {
  ...
  @objc func scaledData(with size: CGSize, isQuantized: Bool) -> NSData? {
    guard let cgImage = self.cgImage else { return nil }
    return UIImage.normalizedData(from: cgImage, resizingTo: size, quantum: Float32.self) as NSData?;
  }
  ...
  /// rotated.
  @objc func transformOrientationToUp() -> UIImage? {
  ...
  @objc private var data: Data? {
    return self.pngData() ?? self.jpegData(compressionQuality: Constant.jpegCompressionQuality)
  }

  @objc static func normalizeImage(_ image: CGImage, resizingTo size: CGSize) -> CGImage? {
